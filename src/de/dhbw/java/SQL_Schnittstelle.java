...
public abstract class SQL_Schnittstelle {
		...
		try {
			...
			while (rs.next()) {
				buchungListe.add(new Buchung(rs.getInt("buchungid"), rs
						...
						.getString("status"), rs
						.getString("veranstaltungsBezeichnung")));
			}
	 ...
	 * @param zeitVon
	 * @param zeitBis
	 * @return Gibt eine Meldung zurück falls es sich um einen besonderen Raum
	 *         handelt und falls nicht wird ein leerer String zurück gegeben
	 * 
	 */
	public static String pruefeBuchungskonflikt(String raumbezeichnung,
		...
		try {
			...
			if (!buchungen.isEmpty()
					&& (raumbezeichnung.equals("Gewölbekeller"))) {
